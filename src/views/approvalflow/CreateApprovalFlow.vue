<template>
  <div class="form-container">
    ...
    <div>
      <label>流程名稱：</label>
      <input v-model="flowName" type="text" placeholder="請輸入流程名稱" class="input-field" />
    </div>
    ...
    <div v-for="(step, index) in approvalSteps" :key="index" class="step-group">
      ...
      <label>請求類型：</label>
      <select v-model="step.requestTypeId" class="select-field" :disabled="index !== 0">
      ...
      <label>員工職位：</label>
      <select v-model="step.employeePositionId" class="select-field" :disabled="index !== 0">
      ...
      <label>簽核人職位：</label>
      <select v-model="step.approverPositionId" class="select-field">
        <option v-for="position in positions" :key="position.positionId" :value="position.positionId">
          {{ position.positionName }}
        </option>
      </select>
    ...
    <button type="button" @click="addStep" class="add-button">新增步驟</button>
    <button type="submit" @click="submitForm" class="submit-button">提交</button>
  </div>
</template>
