<template>
    ...
    <div>
      ...
      <div class="card card-default" id="page-views">
        <div class="card-header">
          <h2>當日考勤</h2>
          <RouterLink class="btn btn-outline-primary btn-pill" to="/attendance/clock">回到打卡頁面</RouterLink>  
        </div>
        <div class="card-body py-0" data-simplebar>
          <!-- 顯示錯誤信息 -->
          <div v-if="error" class="alert alert-danger" role="alert">{{ error }}</div>
          <div v-if="atError" class="alert alert-danger" role="alert">{{ atError }}</div>
          <div v-if="todayAttendance">
            <!-- 當天考勤基本資料 -->
            <h4>
              <span class="badge badge-square badge-outline-primary">今日考勤基本資料</span>
            </h4>
            <table class="table table-borderless table-thead-border">
              ...
              <tbody>
                <tr>
                  ...
                  <td class="text">{{ todayAttendance.hasViolation ? '是' : '否' }}</td>
                </tr>
              </tbody>
            </table>
            <!-- 顯示打卡記錄 -->
            <h4>
              <span class="badge badge-square badge-outline-primary">打卡記錄</span>
            </h4>
            <table class="table table-borderless table-thead-border">
              <thead>
                <tr>
              ...
              <tbody>
                <tr v-for="(log, index) in todayAttendance.attendanceLogs" :key="index">
            ...
            <!-- 顯示異常記錄 -->
            <h4>
              <span class="badge badge-square badge-outline-primary">異常記錄</span>
            </h4>
            <table class="table table-borderless table-thead-border">
              <thead>
                <tr>
              ...
              <tbody>
                <tr v-for="(violation, index) in todayAttendance.attendanceViolations" :key="index">
      ...
      <div class="card card-default" id="page-views">
        ...
        <div class="card-body py-0" data-simplebar>
          ...
          <br />
          <!-- 顯示錯誤信息 -->
          <div v-if="aError" class="alert alert-danger" role="alert">{{ aError }}</div>
          <div v-if="attendance">
            <!-- 當天考勤基本資料 -->
            <h4>
              <span class="badge badge-square badge-outline-primary">今日考勤基本資料</span>
            ...
            <table class="table table-borderless table-thead-border">
              ...
              <tbody>
                <tr>
                  ...
                  <td class="text">{{ attendance.hasViolation ? '是' : '否' }}</td>
                </tr>
              </tbody>
            </table>
            <!-- 顯示打卡記錄 -->
            ...
            <table class="table table-borderless table-thead-border">
              ...
              <tbody>
                <tr v-for="(log, index) in attendance.attendanceLogs" :key="index">
  ...
  const fetchAttendance = async () => {
    try {
      const response = await axios.get(`/api/attendancelogs?date=${queryDate.value}`)
